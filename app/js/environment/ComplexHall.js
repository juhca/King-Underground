...
ComplexHall.prototype.create = function () {
    ...
    function tla() {
        ...
        var groundMaterial = createMaterial(this.scene, 'assets/textures/dirt01.jpg', 'tla', 1.5, 4.0, new BABYLON.Color3.Black());
    ...
    function stena() {
        ...
        var ravno1Wall1 = createWall(this.scene, wallMaterial, 20.5, 1, 91.5, 10, 'ravno1Wall1');
        ravno1Wall1 = properties(ravno1Wall1, -234.5, 9.05, 37.2, 0, 0, 0, 1, 1.1, 1);

        var ravno1Wall2 = createWall(this.scene, wallMaterial, 20.5, 1, 110, 10, 'ravno1Wall2');
        ravno1Wall2 = properties(ravno1Wall2, -240.5, 8.5, 40.2, 0, 0, 0, 1, 1, 1);

        var drugiKlanecWall1 = clone_and_properties(ravno1Wall2, 'drugiKlanecWall1', 50.45, 10, 32.1, 0, (Math.PI/2), 0, 1, 1.2, 1);
        var drugiKlanecWall2 = clone_and_properties(drugiKlanecWall1, 'drugiKlanecWall2', -20.3, 0, 6, 0, 0, 0, 1, 1.2, 1);

        var ravno2Wall1 = clone_and_properties(ravno1Wall2, 'ravno2Wall1', 80.65, 25, 75.1, 0, 0, 0, 1, 1.2, 1);
    ...
    function strop() {
        ...
        var ground = createBox(this.scene, groundMaterial, 5, 2, 40, 'ground');
        ground = properties(ground, -220, 7.0, -1, 0, 0, 0, 1, 1, 1);

        var prviKlanec = clone_and_properties(ground, 'prviKlanec', -17.5, 6.15, 20, 0, (Math.PI/2), -(Math.PI/10), 1, 1, 1);
        var ravno1 = clone_and_properties(ground, 'ravno1', -17.5, 12.5, 58.8, 0, (Math.PI/2), 0, 1, 1, 1);
        var drugiKlanec = clone_and_properties(ground, 'drugiKlanec', 0.8, 18.0, 76.3, 0, (Math.PI), -(Math.PI/12), 1, 1, 1);
        var ravno2 = clone_and_properties(ground, 'ravno2', 39.65, 23.155, 76.3, 0, 0, 0, 1, 1, 1);
        var tretjiKlanec = clone_and_properties(ground, 'tretjiKlanec', 57.15, 29.60, 96, 0, (Math.PI/2), -(Math.PI/10), 1.1, 1, 1);
        var ravno3 = clone_and_properties(ground, 'ravno3', 57.15, 37, 136.65, 0, (Math.PI/2), 0, 1, 1, 1);
        var cetrtiKlanec = clone_and_properties(ground, 'cetrtiKlanec', 40, 42.5, 154.2, 0, 0, -(Math.PI/11), 1, 1, 1);
        var ravno4 = clone_and_properties(ground, 'ravno4', 1.15, 48, 154.2, 0, 0, 0, 1, 1, 1.5);
        var ravno5 = clone_and_properties(ground, 'ravno5', -38.15, 48, 154.2, 0, 0, 0, 1.5, 1.0, 1.5);
    }
}